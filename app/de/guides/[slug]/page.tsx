...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!g) return {};
  const canonical = `/de/guides/${slug}`;
  return {
    title: { absolute: g.metaTitle },
    description: g.metaDescription,
    alternates: { canonical },
    openGraph: { title: g.metaTitle, description: g.metaDescription, url: canonical, type: "article", locale: "de_DE" },
  };
}
...
export default async function GermanGuidePage({ params }: Props) {
  ...
  const url = `https://hyrde.net/de/guides/${slug}`;
  const articleJsonLd = {
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Startseite", item: "https://hyrde.net/de" },
      { "@type": "ListItem", position: 2, name: "Ratgeber", item: "https://hyrde.net/de/guides" },
      { "@type": "ListItem", position: 3, name: g.title, item: url },
    ],
  };

  const related = g.related.map(s => DE_GUIDES[s]).filter(Boolean);
    ...
    <div lang="de" className="min-h-screen bg-surface-gray">
      ...
      <article className="max-w-[760px] mx-auto px-6 md:px-8 pt-20 pb-16">
        <nav className="text-xs font-body text-on-surface-variant mb-6 flex items-center gap-2 flex-wrap">
          <Link href="/de" className="hover:text-electric-violet transition-colors">Startseite</Link>
          <span>/</span>
          <Link href="/de/guides" className="hover:text-electric-violet transition-colors">Ratgeber</Link>
          <span>/</span>
        ...
        <p className="text-xs font-body text-on-surface-variant inline-flex items-center gap-4 mb-8">
          ...
          <span className="inline-flex items-center gap-1.5">
            ...
            Aktualisiert {new Date(g.updated).toLocaleDateString("de-DE", { month: "long", year: "numeric" })}
          </span>
        </p>
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-bold font-headline text-on-surface mb-5">Häufige Fragen</h2>
        ...
        {related.length > 0 && (
          <section className="border-t border-border-crisp pt-8">
            <h2 className="text-lg font-bold font-headline text-on-surface mb-4">Weiterlesen</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {related.map(r => (
                <Link key={r.slug} href={`/de/guides/${r.slug}`}
