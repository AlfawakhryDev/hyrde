...
import { SKILLS, CITIES, MOCK_FREELANCERS, ALL_SKILL_SLUGS, ALL_CITY_SLUGS, isIndexedCityPage, getRate, getRateContext, getSkillCityIntro, getSkillCityFaqs, getCityRateComparisons } from "@/content/data";
import FreelancerCard from "@/components/marketing/FreelancerCard";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Hire a ${s.label} ${loc}. $${rate}/hr avg`;
  const description = `Find pre-vetted ${s.label}s ${loc}. Local average rate $${rate}/hr. AI-matched in 60 seconds, free of Hyrde fees on your first three projects. No Connects, no fake reviews.`;
  ...
  return {
    ...
    // Curated indexing (see INDEXED_CITY_PAGES in src/content/data.ts): pages with
          ...
          <div>
            ...
            <div className="bg-white rounded-xl p-5 border border-border-crisp mb-6">
              ...
              <p className="text-xs font-body text-on-surface-variant mt-3 pt-3 border-t border-border-crisp">
                Platform fee:{" "}
                <span className="text-electric-violet font-semibold">$0 on your first 3 projects</span>{" "}
