...
export function getSkillCityIntro(skillSlug: string, citySlug: string): string {
  ...
  const variants = [
    ...
    `Hiring a ${s.label} ${locShort} typically runs about $${ctx.rate}/hr. Hyrde's AI reads your brief and surfaces a ranked shortlist of vetted ${s.label}s ${c.label === "Remote" ? "from around the world" : `across ${c.region}`} in seconds — no Connects to buy, no bidding wars, and it's free of Hyrde fees on your first three projects instead of the 20%+ you'd lose elsewhere.`,
    `Need a ${s.label} ${locShort}? With ${s.demand} demand for ${s.category.toLowerCase()} skills, finding reliable talent fast matters. Hyrde pre-vets every ${s.label}, AI-matches them to your brief in about 60 seconds, and is free of Hyrde fees on your first three projects — versus the effective 22–34% take rate burned clients report on legacy platforms.`,
  ];
  ...
  return [
    {
      ...
      a: `The going market rate for a ${s.label} ${loc} is around $${ctx.rate}/hr. Junior talent starts near $${ctx.junior}/hr, while senior specialists run to about $${ctx.senior}/hr. On Hyrde, your first three projects carry no Hyrde fee — not the 20–34% effective take rate clients report on Upwork and Fiverr.`,
    },
    {
    ...
    },
    {
      ...
      a: `No. Posting a brief, getting matched, and reviewing ${s.label} candidates ${loc} is completely free. Hiring is free of Hyrde fees on your first three projects too — and every match is pre-vetted, so you're not gambling on fake reviews or padded badges.`,
    },
  ];
}
